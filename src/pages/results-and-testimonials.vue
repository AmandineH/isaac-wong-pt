      ...
      >
        ...
        <div class="bg-black shadow-md rounded-12px p-16px">
          ...
          <client-only>
            ...
            >
              Service(s):
              <span v-for="(category, key) in testimonial.category" :key="key">
                {{
                  category
                    .split("-")
                    .join(" ")
    ...
    testimonials() {
      ...
      if (category) {
        ...
        if (category === "online-coaching")
          filteredTestimonials = onlineTestimonials;
        if (category === "mentorship")
          filteredTestimonials = mentorshipTestimonials;
        if (category === "personal-training")
          filteredTestimonials = ptTestimonials;

        return filteredTestimonials.map((t, index) => ({
          ...t,
